...
import { 
  ...
  TrendingUp, X, Activity, Shield, Search, 
  LayoutDashboard, UploadCloud, Send, AlertCircle, 
  CheckCircle2, Ticket, Loader2
} from 'lucide-react';
...
const TicketSubmissionForm = ({ onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log("DEBUG: Submitting ticket for user:", currentUser.id);
    ...
    if (!formData.resourceLocation) errors.resourceLocation = 'Location is required';
    if (!formData.description) errors.description = 'Description is required';
    if (!emailRegex.test(formData.contactEmail)) {
      errors.contactEmail = 'Invalid email format (e.g. name@domain.com)';
    }
    ...
    if (Object.keys(errors).length > 0) {
      ...
      setErrorMessage('Please fix the highlighted errors before transmitting.');
      return;
    }
    ...
    try {
      ...
      if (res.status === 201 || res.status === 200) {
        ...
        setTimeout(() => { if (onSuccess) onSuccess(res.data.id); }, 1500);
      }
    } catch (err) {
      console.error("DEBUG: Submission error", err);
      setSubmitStatus('error');
      setErrorMessage(err.response?.data?.message || err.response?.data || 'Server Uplink Failure. Please check connection.');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white rounded-[3rem] overflow-hidden shadow-2xl shadow-gray-200/50 border border-gray-100">
      ...
      <form onSubmit={handleSubmit} className="p-10 space-y-8">
        ...
        {errorMessage && (
            <div className="bg-rose-50 text-rose-700 p-6 rounded-2xl flex items-start gap-4 border border-rose-100 animate-fade-in">
                <AlertCircle className="w-6 h-6 mt-0.5 flex-shrink-0" />
                <div>
                    <p className="font-black text-rose-800 uppercase tracking-widest text-xs mb-1">Attention Required</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-2">
            ...
            <select name="category" value={formData.category} onChange={handleInputChange}
              className={`w-full bg-gray-50 border ${fieldErrors.category ? 'border-rose-400' : 'border-gray-100'} text-gray-900 rounded-2xl px-5 py-4 text-sm font-bold focus:ring-2 focus:ring-blue-500/20 outline-none transition-all appearance-none shadow-sm cursor-pointer`}>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <select name="priority" value={formData.priority} onChange={handleInputChange}
                className="w-full bg-gray-50 border border-gray-100 text-gray-900 rounded-2xl px-5 py-4 text-sm font-bold focus:ring-2 focus:ring-blue-500/20 outline-none transition-all appearance-none shadow-sm cursor-pointer">
          ...
          <div className="md:col-span-2 space-y-2">
            ...
            <input type="text" name="resourceLocation" value={formData.resourceLocation} onChange={handleInputChange}
              placeholder="e.g. Block C, Level 2, Room 405"
              className={`w-full bg-gray-50 border ${fieldErrors.resourceLocation ? 'border-rose-400' : 'border-gray-100'} text-gray-900 rounded-2xl px-5 py-4 text-sm font-bold focus:ring-2 focus:ring-blue-500/20 outline-none transition-all shadow-sm`} />
          </div>
          ...
          <div className="space-y-2">
            ...
            <input type="email" name="contactEmail" value={formData.contactEmail} onChange={handleInputChange}
              ...
              className={`w-full bg-gray-50 border ${fieldErrors.contactEmail ? 'border-rose-400' : 'border-gray-100'} text-gray-900 rounded-2xl px-5 py-4 text-sm font-bold focus:ring-2 focus:ring-blue-500/20 outline-none transition-all shadow-sm`} />
            {fieldErrors.contactEmail && <p className="text-[10px] font-bold text-rose-500 ml-2">{fieldErrors.contactEmail}</p>}
          </div>
          ...
          <div className="space-y-2">
            ...
            <input type="tel" name="contactPhone" value={formData.contactPhone} onChange={handleInputChange}
              ...
              className={`w-full bg-gray-50 border ${fieldErrors.contactPhone ? 'border-rose-400' : 'border-gray-100'} text-gray-900 rounded-2xl px-5 py-4 text-sm font-bold focus:ring-2 focus:ring-blue-500/20 outline-none transition-all shadow-sm`} />
            {fieldErrors.contactPhone && <p className="text-[10px] font-bold text-rose-500 ml-2">{fieldErrors.contactPhone}</p>}
          </div>
          ...
          <div className="md:col-span-2 space-y-2">
            ...
            <textarea name="description" value={formData.description} onChange={handleInputChange} rows="5"
              ...
              className={`w-full bg-gray-50 border ${fieldErrors.description ? 'border-rose-400' : 'border-gray-100'} text-gray-900 rounded-[2rem] px-6 py-5 text-sm font-bold focus:ring-2 focus:ring-blue-500/20 outline-none resize-none transition-all shadow-sm leading-relaxed`} />
          </div>
        </div>
    ...
    <div className="space-y-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in pb-20">
      ...
      <div className="animate-fade-in">
        {activeTab === 'list' ? (
          ...
          <TicketSubmissionForm onSuccess={handleTicketSubmitted} />
        )}
      </div>
    </div>
  );
