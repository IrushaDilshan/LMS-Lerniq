...
function AppContent() {
  ...
  const navLinks = useMemo(() => [
    ...
    { name: 'Role Management', path: '/admin/roles', icon: <Shield className="w-5 h-5 flex-shrink-0" />, roles: ['ADMIN'] },
    { name: 'Settings', path: '/settings', icon: <Settings className="w-5 h-5 flex-shrink-0" />, roles: ['USER', 'ADMIN', 'TECHNICIAN'] },
  ].filter(link => link.roles.includes(currentUser.role)), [currentUser.role]);
        ...
        <header className="h-[80px] bg-white border-b border-gray-100 flex items-center justify-between px-8 relative z-20">
          ...
          <div className="flex items-center gap-10">
            ...
            <div className="flex items-center gap-3 pr-6 border-r border-gray-100">
               <button onClick={() => setIsNotificationOpen(true)} className="p-2.5 text-gray-400 hover:text-[#061224] transition-colors relative">
        ...
        <div className="flex-1 overflow-y-auto bg-[#F8FAFC]">
          <div className="p-8 pb-20">
            ...
            <Routes>
              ...
              <Route path="/admin/roles" element={currentUser.role === 'ADMIN' ? <AdminRoleManagementPage /> : <Navigate to="/" replace />} />
